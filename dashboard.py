...
def load_data():
    ...
    df_izm_group = pd.read_excel(path_to_data, sheet_name="IZM_group")
    df_izm_group["Дата"] = df_izm_group["Дата"].apply(pd.to_datetime)

    df_productivity_group = pd.read_excel(path_to_data, sheet_name="Productivity_group")
